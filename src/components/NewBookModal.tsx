...
export const NewBookModal: React.FC<NewBookModalProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div className="bg-emerald-600 text-white p-4 sm:p-5 md:p-6 flex items-center justify-between">
          <div className="flex items-center gap-2.5 sm:gap-3">
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-white/20 flex items-center justify-center text-white flex-shrink-0">
              <BookPlus className="w-6 h-6 sm:w-7 sm:h-7" />
            </div>
            <div>
              <span className="text-emerald-200 text-xs sm:text-sm font-bold uppercase tracking-wider">
                독서 도장판
              </span>
              <h2 className="text-lg sm:text-2xl md:text-3xl font-black tracking-tight leading-tight">
                새 책 추천하기
              </h2>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 md:p-8 space-y-4 sm:space-y-6 overflow-y-auto flex-1">
          ...
          <div>
            <label className="text-base sm:text-xl md:text-2xl font-black text-stone-900 flex items-center gap-2 mb-2">
              <span className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-emerald-600 text-white text-sm sm:text-base flex items-center justify-center font-black">1</span>
              <span>추천할 책 제목</span>
            </label>
            <input
              ...
              className="w-full p-3 sm:p-4 rounded-2xl border-2 border-stone-300 text-base sm:text-xl md:text-2xl font-bold focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 text-stone-900"
              maxLength={40}
            />
          </div>
          ...
          <div>
            <label className="text-base sm:text-xl md:text-2xl font-black text-stone-900 flex items-center gap-2 mb-2">
              <span className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-emerald-600 text-white text-sm sm:text-base flex items-center justify-center font-black">2</span>
            ...
            <div className="flex gap-2 mb-2.5">
              <button
                ...
                className={`flex-1 min-h-[44px] py-2.5 px-2 rounded-xl font-black text-sm sm:text-base md:text-lg border-2 transition cursor-pointer ${
            ...
            {recommenderType === 'student' && (
              <div className="bg-stone-50 p-2 sm:p-3 rounded-2xl border-2 border-stone-200">
                <div className="grid grid-cols-5 sm:grid-cols-8 md:grid-cols-10 gap-1.5 sm:gap-2 max-h-36 overflow-y-auto p-0.5 sm:p-1">
                  {Array.from({ length: studentCount }, (_, i) => i + 1).map((num) => {
                    ...
                    return (
                      <button
                        ...
                        className={`min-h-[40px] rounded-xl font-black text-xs sm:text-sm md:text-base transition-all cursor-pointer flex items-center justify-center ${
                          isSelected
                ...
                <div className="mt-2 pt-2 border-t border-stone-200 flex items-center gap-2">
                  ...
                  <input
                    ...
                    className="w-20 px-2 py-1.5 bg-white border border-stone-300 rounded-lg text-base font-bold text-stone-800"
                  />
          ...
          <div>
            <label className="text-base sm:text-xl md:text-2xl font-black text-stone-900 flex items-center gap-2 mb-2">
              <span className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-emerald-600 text-white text-sm sm:text-base flex items-center justify-center font-black">3</span>
            ...
            <input
              ...
              className="w-full p-3 sm:p-3.5 rounded-2xl border-2 border-stone-300 text-base sm:text-lg font-medium focus:outline-none focus:border-emerald-500 text-stone-900"
              maxLength={80}
            />
          </div>
          ...
          {/* 4. Color Theme */}
          <div>
            <label className="text-xs sm:text-sm font-black text-stone-700 block mb-2">
              카드 색상 테마
            </label>
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
              {COLOR_OPTIONS.map((c) => (
                <button
                  ...
                  className={`min-h-[44px] p-2 rounded-xl border-2 text-xs sm:text-sm font-black transition cursor-pointer flex flex-col items-center justify-center gap-1 ${c.bgClass} ${
          ...
          <button
            ...
            type="submit"
            className="min-h-[52px] w-full py-3.5 sm:py-4 md:py-5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-black text-lg sm:text-xl md:text-2xl rounded-2xl shadow-xl shadow-emerald-700/25 active:scale-98 transition-all flex items-center justify-center gap-2 sm:gap-3 cursor-pointer"
          >
            <BookPlus className="w-6 h-6 sm:w-7 sm:h-7 stroke-[2.5]" />
